...
void two_opt(uint_fast16_t n, vi& tour, uint_fast16_t* position, const clock_t& clock_begin);
...
void three_opt(int n, vi& tour, uint_fast16_t* position, const clock_t& clock_begin);
...
bool is_time_exceeded(clock_t begin) {
    return (compute_time(begin) > TIME_LIMIT_THRESHOLD);
}
...
void fill_up_dist_matrix(uint_fast16_t n, const vdd& coords) {
    ...
    for (uint_fast16_t i = 0; i < n - 1; ++i) {
        for (j = i + 1; j < n; ++j) {
            ...
            xd = (x1 - x2);
            yd = (y1 - y2);
            dist[i][j] = dist[j][i] = round(sqrt(xd*xd + yd*yd));
        }
    }
}
...
vi local_search(uint_fast16_t n, const clock_t& clock_begin) {
    ...
    // Diversification
    while (!is_time_exceeded(clock_begin)) {
        ...
        // Intensification
        two_opt(n, tour, position, clock_begin);
        three_opt(n, tour, position, clock_begin);
...
void two_opt(uint_fast16_t n, vi& tour, uint_fast16_t* position, const clock_t& clock_begin) {
     ...
     * further
     */
    while (!is_time_exceeded(clock_begin) && improvingTSP) {
        improvingTSP = false;
